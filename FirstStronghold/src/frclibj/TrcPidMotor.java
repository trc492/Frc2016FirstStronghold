...
public class TrcPidMotor implements TrcTaskMgr.Task
{
            ...
            double timeout)
    {
        ...
        if (timeout != 0.0)
        {
            expiredTime += Timer.getFPGATimestamp();
        }
            ...
            boolean stopPid)
    {
        ...
        if ((flags & PIDMOTORF_STALLED) != 0)
        {
            if (power == 0.0)
            {
                ...
                //
                if (resetTimeout == 0.0 ||
                    Timer.getFPGATimestamp() - prevTime > resetTimeout)
                {
        ...
        else
        {
            ...
            if (stallMinPower > 0.0 && stallTimeout > 0.0)
            {
                ...
                if (Timer.getFPGATimestamp() - prevTime > stallTimeout)
                {
                    //
    ...
    public void postPeriodicTask(TrcRobot.RunMode runMode)
    {
        ...
        else if ((flags & PIDMOTORF_ENABLED) != 0)
        {
            if ((flags & PIDMOTORF_HOLD_TARGET) == 0 && pidCtrl.isOnTarget() ||
                expiredTime != 0.0 && Timer.getFPGATimestamp() >= expiredTime)
            {
                stop(true);
                if (notifyEvent != null)
                {
